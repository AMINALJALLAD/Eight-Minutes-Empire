#include "Greedy.h"
#include <ctime>
Greedy::Greedy() :PlayerAction() {

}


void Greedy::placeNewArmies(std::string color,std::vector <Country*>* owned, Map* world, std::string countryStr, int numOfArmiesPlacing, bool samePlace) {
...
void Greedy::moveOverLand(std::string color, std::vector <Country*>* owned, Map* world, std::string origin_str, std::string destination_str, int numOfArmiesMoving)
{
...
void Greedy::moveOverSea(std::string color, std::vector <Country*>* owned, Map* world, std::string origin_str, std::string destination_str, int numOfArmiesMoving)
{
...
void Greedy::buildCity(std::string color, std::vector <Country*>* owned, Map* world, std::string countryStr) {
	//std::cout << "\nin Build city\n";
...
void Greedy::destroyArmy(std::string color, std::vector <Country*>* owned, Map* map, std::string* countryStr, std::string* enemyColor) {
...
bool Greedy::selectCard(std::string color, int index, int& coins,Hand* hand, std::vector <Country*>* owned, Map* map, Deck* deck, std::string* changedCountry, std::string* changedPlayer, std::string* message) {
	//std::cout << "\nin select card method\n";
	bool done = false, ignore = false; int input = 0;
		int choice;
		Card* selected = NULL;
		if (hasBuild(deck, choice) && coins > getCardCost(choice)) {
		...
		else if (hasDestroy(deck, choice) && coins > getCardCost(choice)) {
			selected = deck->isPossibleDrawCard(choice);
			if (possibleAction(selected->getAction(), map, owned, color)) {
				selected = deck->draw(choice - 1);
				hand->addCard(selected);
			...
			*message = "The " + color + " player selected the " + std::to_string(choice) + ", card, he would pay " + std::to_string(getCardCost(choice)) + " coins\nThe action that he chose was " + selected->getAction() + "\n";
			...
			if (!ignore) {
				...
				*message = "The turn of the " + color + " was skipped because all actions cant be done";
			}
...
void Greedy::doAction(std::string color, Map* map, std::vector <Country*>* owned, std::string actionString, std::string* changedCountry, std::string* changedPlayer) {
...
bool Greedy::possibleAction(std::string actionString, Map* map, std::vector <Country*>* owned, std::string color) {
	//std::cout << "in possible action\n\n";
...
bool Greedy::hasBuild(Deck* deck, int& index) {
...
bool Greedy::hasDestroy(Deck* deck, int& index) {
	for (size_t i = 0; i < deck->getBoardCards()->size(); i++) {
...
bool Greedy::hasMoveSea(Deck* deck, int& index) {
	for (size_t i = 0; i < deck->getBoardCards()->size(); i++) {
...
bool Greedy::hasMoveLand(Deck* deck, int& index) {
	for (size_t i = 0; i < deck->getBoardCards()->size(); i++) {
...
bool Greedy::hasplace(Deck* deck, int& index) {
	for (size_t i = 0; i < deck->getBoardCards()->size(); i++) {
...
int Greedy::getCardCost(int choice) {
	switch (choice) {
	case 6:
		return 3;
		break;
...
bool Greedy::contains(std::vector<std::string>* someVector, std::string value) {
...
bool Greedy::someCardPlayable(Deck* deck, int& coins, Map* map, std::vector<Country*>* owned, std::string color)
{
	//std::cout << "\nin someCardPlayable\n";
	for (size_t i = 0; i < deck->getBoardCards()->size(); i++) {
...
std::vector<Card*>* Greedy::getCardPlayable(Deck* deck, int& coins, Map* map, std::vector<Country*>* owned, std::string color)
{
	//std::cout << "\nin getCardPlayable\n";
...
int Greedy::getIndex(std::vector<Card*>* boardCards, Card* card)
{
	//std::cout << "\nin getCardPlayable\n";
